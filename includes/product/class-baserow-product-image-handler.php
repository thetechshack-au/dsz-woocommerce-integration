 ...
 * Description: Handles all product image operations
 * Version: 1.4.0
...
class Baserow_Product_Image_Handler {
    ...
    private $max_image_size = 10485760; // 10MB in bytes

    /**
     * Process all images for a product
     */
    public function process_product_images($image_urls, $product_id) {
        $this->log_debug("Processing product images", array(
        ...
        foreach ($image_urls as $index => $url) {
            ...
            if (!is_wp_error($result)) {
                ...
                $this->log_error("Failed to process image", array(
                    'url' => $url,
    ...
    private function process_single_image($image_url, $product_id) {
        $this->log_debug("Processing single image", array(
            'url' => $image_url,
            'product_id' => $product_id
        ));
        ...
        if ($existing_image) {
            $this->log_debug("Image already exists in media library", array(
     ...
     * Check if image already exists in media library
     */
    private function get_existing_image($image_url) {
        global $wpdb;
    ...
    private function download_image($url) {
        $this->log_debug("Downloading image", array('url' => $url));
        ...
        if (is_wp_error($temp_file)) {
            $this->log_error("Failed to download image", array(
                'url' => $url,
    ...
    private function validate_image_file($file_path) {
        ...
        if ($file_size > $this->max_image_size) {
            return new WP_Error(
                'image_too_large',
                sprintf('Image file size (%s) exceeds maximum allowed size (%s)',
     ...
     */
    private function upload_to_media_library($file_path, $source_url, $product_id) {
        ...
        if (is_wp_error($attachment_id)) {
            $this->log_error("Failed to upload image to media library", array(
        ...
        $this->log_debug("Image uploaded successfully", array(
     ...
     * Set product images
     */
    public function set_product_images($product, $image_ids) {
        if (empty($image_ids)) {
            return;
        }
        ...
        $this->log_debug("Product images set", array(
    ...
    public function cleanup_orphaned_images() {
        ...
        foreach ($orphaned_images as $image) {
            ...
            $this->log_debug("Deleted orphaned image", array(
                'image_id' => $image->ID,
                'title' => $image->post_title
            ));
        }

        return count($orphaned_images);
    }
}
