...
class Baserow_API_Handler {
    ...
    private $timeout = 15; // Reduced timeout to 15 seconds
    ...
    public function get_categories() {
        ...
        try {
            // Make a single request with a larger page size
            $url = trailingslashit($this->api_url) . "api/database/rows/table/{$this->table_id}/?user_field_names=true&fields=Category&size=200";
            ...
            if (is_wp_error($response)) {
                ...
                $stale_cache = get_transient('baserow_categories_stale');
                if ($stale_cache !== false) {
                    Baserow_Logger::info("Returning stale cached categories due to API error");
                    return $stale_cache;
                }
            ...
            set_transient('baserow_categories_stale', $categories, DAY_IN_SECONDS); // Stale cache for fallback

            return $categories;

        } catch (Exception $e) {
            Baserow_Logger::error("Exception in get_categories: " . $e->getMessage());
            
            // Check if we have stale cache
            $stale_cache = get_transient('baserow_categories_stale');
            if ($stale_cache !== false) {
                Baserow_Logger::info("Returning stale cached categories due to exception");
                return $stale_cache;
            }
